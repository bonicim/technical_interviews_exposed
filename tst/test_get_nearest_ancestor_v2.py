...
def test_easy_case():
    assert get_nearest_ancestor_v2(left_child, right_child) == root

def test_ancerstor_2nd_level():
    assert get_nearest_ancestor_v2(child1, child2) == left_child

def test_ancerstor_unbalanced():
    assert get_nearest_ancestor_v2(child1, child2_1_1) == left_child

def test_ancestor_unbalanced_v2():
    assert get_nearest_ancestor_v2(right_child, child2_1_1) == root

def test_ancestor_unbalanced_v3():
    assert get_nearest_ancestor_v2(child2_1, child2_1_1) == child2

root = Node(42, parent=None)
